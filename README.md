...
Park Directories is a directory bookmarking tool. You assign short names to
directories you visit often, then jump to them instantly:

```
pd -a work C:\Users\you\projects\work   # bookmark a directory
pd work                                  # jump there instantly
pd work/src/components                   # jump to a subdirectory of the bookmark
pd -l                                    # list all bookmarks
pd -d work                               # remove a bookmark
```

The core is a compiled Rust binary (`pd.exe` on Windows, `pd` on Linux). A thin
shell function — generated by the binary itself — handles the `cd` call, since no
external process can change your shell's working directory directly.
...
format is plain text — one bookmark per line — and is human-readable and
hand-editable:
...
`PD_DATA_FILE` environment variable or pass `--data-file <path>` to any command.

---

### Updating the integration file

When you update the `pd` binary, regenerate `pd.nu` to pick up any changes to
...
The same Rust binary handles all logic. A thin bash function — generated by
`pd init bash` — performs the `cd` call, since no external process can change
your shell's working directory directly.
...
When you update the `pd` binary, re-run the eval line to pick up any changes
to the shell integration. The simplest way is to restart your terminal, or:
